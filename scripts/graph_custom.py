...
def generate_ego_graph(graph, radius=5, distance=None, verbose=True):
    netx = graph.copy()
    delayed_subgraphs = []

    for n in tqdm(netx, total=len(netx), disable=not verbose):
        delayed_subgraphs.append(delayed(generate_subgraph)(netx, n, radius, distance))
...
def meshedness(graph, ego_graph_collection, radius=5, name="meshedness", distance=None, verbose=True):
    """
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(ego_graph_collection), disable=not verbose):
            sub = ego_graph_collection[key]
            
            netx.nodes[key][name] = _meshedness(
                sub
...
def mean_node_dist(graph, name="meanlen", length="mm_len", verbose=True):
    ...
    for n, nbrs in tqdm(netx.adj.items(), total=len(netx), disable=not verbose):
        ...
        netx.nodes[key][name] = np.mean(lengths)
...
def cds_length(
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(ego_graph_collection), disable=not verbose):
            ...
            netx.nodes[key][name] = _cds_length(
...
def mean_node_degree(
    graph, ego_graph_collection, radius=5, name="mean_nd", degree="degree", distance=None, verbose=True
):
    """
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
            sub = ego_graph_collection[key]
            netx.nodes[key][name] = _mean_node_degree(sub, degree=degree)
...
def proportion(
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
            ...
            if three:
                netx.nodes[key][three] = counts[3] / len(sub)
            if four:
                netx.nodes[key][four] = counts[4] / len(sub)
            if dead:
                netx.nodes[key][dead] = counts[1] / len(sub)
        return netx
...
def cyclomatic(graph, ego_graph_collection, radius=5, name="cyclomatic", distance=None, verbose=True):
    """
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
            sub = ego_graph_collection[key]
            netx.nodes[key][name] = _cyclomatic(
                sub
            )  # save value calulated for subgraph to node
...
def edge_node_ratio(
    graph, ego_graph_collection, radius=5, name="edge_node_ratio", distance=None, verbose=True
):
    """
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
            sub = ego_graph_collection[key]
            netx.nodes[key][name] = _edge_node_ratio(
                sub
            )  # save value calulated for subgraph to node
...
def gamma(graph, ego_graph_collection, radius=5, name="gamma", distance=None, verbose=True):
    """
...
def local_closeness_centrality(
    graph, ego_graph_collection, radius=5, name="closeness", distance=None, weight=None
):
    """
...
def closeness_centrality(
    ...
    **kwargs
    ...
    if radius:
        ...
        for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
            sub = ego_graph_collection[key]
            netx.nodes[key][name] = _closeness_centrality(
                sub, key, length=weight, len_graph=lengraph
            )
    else:
    ...
    **kwargs
    ...
    if radius:
        for n in tqdm(G, total=len(G), disable=not verbose):
            ...
            netx.nodes[key][name] = nx.betweenness_centrality(
...
def straightness_centrality(
    ...
    if radius:
        for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
            sub = ego_graph_collection[key]
            netx.nodes[key][name] = _straightness_centrality(
...
def local_straightness_centrality(
    graph, ego_graph_collection, radius=5, name="straightness", distance=None, weight="mm_len"
):
    """
    ...
    return straightness_centrality(
        graph=graph, ego_graph_collection = ego_graph_collection, radius=radius, name=name, distance=distance, weight=weight
    )
...
def subgraph(
    ...
    for key in tqdm(ego_graph_collection, total=len(netx), disable=not verbose):
        ...
        if meshedness:
            netx.nodes[key]["meshedness"] = _meshedness(sub)
        ...
        if cds_length:
            ...
            netx.nodes[key]["cds_length"] = _cds_length(sub, mode=mode, length=length)

        if mean_node_degree:
            netx.nodes[key]["mean_node_degree"] = _mean_node_degree(sub, degree=degree)

        if proportion:
            counts = _proportion(sub, degree=degree)
            if proportion[3]:
                netx.nodes[key]["proportion_3"] = counts[3] / len(sub)
            if proportion[4]:
                netx.nodes[key]["proportion_4"] = counts[4] / len(sub)
            if proportion[0]:
                netx.nodes[key]["proportion_0"] = counts[1] / len(sub)

        if cyclomatic:
            netx.nodes[key]["cyclomatic"] = _cyclomatic(sub)

        if edge_node_ratio:
            netx.nodes[key]["edge_node_ratio"] = _edge_node_ratio(sub)

        if gamma:
            netx.nodes[key]["gamma"] = _gamma(sub)

        if local_closeness:
            lengraph = len(netx)
            netx.nodes[key]["local_closeness"] = _closeness_centrality(
                sub, key, length=closeness_weight, len_graph=lengraph
            )
